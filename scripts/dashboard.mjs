...
//
//   GET /               dashboard (cost cards, live activity, todos)
//   GET /api/cost       totals + per-session breakdown
...
<style>
 ...
 .stat{flex:1;min-width:130px;background:#0d1117;border:1px solid #30363d;border-radius:6px;padding:10px}
 .stat b{display:block;font-size:22px;font-weight:600}
 .stat span{color:#8b949e;font-size:12px}
 .item{padding:4px 0;border-bottom:1px solid #21262d;font-size:13px;word-break:break-word}
...
<div class="card"><h3>Live activity</h3><div id="activity"></div></div>
...
function item(x){
  ...
  var body='';
  if(x.type==='tool'){body=esc(x.tool)+' ['+esc(x.status)+'] '+esc((x.cmd||'').slice(0,90));}
  else if(x.type==='reasoning'||x.type==='text'){body=esc((x.text||'').slice(0,140));}
