...
# v13: caches API keys in .env.local (mode 0600, gitignored) so re-runs
#      are silent. Uses existing node user (UID 1000). Saves versions in scripts/.
...
load_env_file() {
    ...
    # Parse KEY=VALUE lines without sourcing (avoids arbitrary code execution)
...
save_env_file() {
    # Write with mode 0600 before content so file is never world-readable
    touch "$ENV_FILE"
    chmod 600 "$ENV_FILE"
...
main() {
    ...
    # Load cached keys (env vars take precedence)
    load_env_file
...
jev-guard not loading: run opencode plugin add jev-guard@0.2.1.
...
DOCKERIGNORE_EOF
    ...
    # 9. GITIGNORE (includes .env.local)
...
GITIGNORE_EOF
    ...
    # 11. SECRET SCAN (excludes .env.local and .git)
    echo ""
    echo "=== Secret Scan ==="
    SECRET_FOUND=0
    ...
    if [ -n "$(git -C "$REPO_DIR" status --porcelain)" ]; then
        git -C "$REPO_DIR" commit -m "OpenCode + DeepSeek V4.1 Flash + Jev: v13"
        if [ $? -ne 0 ]; then
    ...
    if [ -n "$(git -C "$REPO_DIR" status --porcelain)" ]; then
        git -C "$REPO_DIR" commit -m "Add verification log v13"
    fi
    ...
    echo "Script v13:  $BASE/scripts/stage-opencode-repo-v13.sh"
