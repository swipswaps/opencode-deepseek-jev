...
# v12: uses existing node user/group (UID/GID 1000) in node:22-bookworm-slim.
#      Saves versioned staging scripts into scripts/.
...
main() {
    ...
    # Copy current and previous staging scripts into scripts/
    if [ -f "$REPO_DIR/stage-opencode-repo-v12.sh" ]; then
        cp "$REPO_DIR/stage-opencode-repo-v12.sh" "$SCRIPTS_DIR/stage-opencode-repo-v12.sh"
    fi
    if [ -f "$REPO_DIR/stage-opencode-repo-v11.sh" ]; then
        cp "$REPO_DIR/stage-opencode-repo-v11.sh" "$SCRIPTS_DIR/stage-opencode-repo-v11.sh"
    fi
...
# Use the existing node user (UID 1000, GID 1000) already present in the base image.
# Do NOT create another user with the same UID/GID.
...
# Install jev-guard via OpenCode V2 plugin CLI (pinned version)
RUN opencode plugin add jev-guard@0.2.1
...
COMPOSE_EOF
    ...
    # 3. OPENCODE.JSON (V2 plugins key; jev-guard installed via CLI, not array)
...
- Uses the existing node user (UID 1000, GID 1000)
- Repo mounted at /workspace
- jev-guard: installed via opencode plugin add jev-guard@0.2.1
...
Cause:   Plugin not installed via opencode plugin add
Fix:     Dockerfile runs opencode plugin add jev-guard@0.2.1

Symptom: docker build permission denied
Cause:   User not in docker group
Fix:     sudo usermod -aG docker $USER && newgrp docker

Symptom: groupadd: GID '1000' already exists
Cause:   node:22-bookworm-slim already has node user/group with UID/GID 1000
Fix:     Use the existing node user; do not create a new user with UID/GID 1000
README_EOF
...
GITIGNORE_EOF
    ...
    # 14. BEHAVIORAL SMOKE TEST: Plugin list
    echo ""
    ...
    # 15. BEHAVIORAL SMOKE TEST: MCP list
    echo ""
    ...
    if [ -n "$(git -C "$REPO_DIR" status --porcelain)" ]; then
        git -C "$REPO_DIR" commit -m "OpenCode + DeepSeek V4.1 Flash + Jev: v12"
        if [ $? -ne 0 ]; then
    ...
    # 17. GH REPO CREATE OR SYNC (idempotent)
    echo ""
    echo "=== GitHub Sync ==="
    ...
    echo "REPORT 4: jev-guard installed via CLI in Dockerfile" | tee -a "$VERIFY_LOG"
    grep -q "opencode plugin add jev-guard" "$REPO_DIR/docker/Dockerfile"
    if [ $? -eq 0 ]; then
        echo "  PASS" | tee -a "$VERIFY_LOG"
    else
        echo "  FAIL" | tee -a "$VERIFY_LOG"
    ...
    echo "REPORT 10: scripts/ directory contains versioned scripts" | tee -a "$VERIFY_LOG"
    if [ -f "$SCRIPTS_DIR/stage-opencode-repo-v12.sh" ]; then
        echo "  PASS" | tee -a "$VERIFY_LOG"
    else
        echo "  FAIL" | tee -a "$VERIFY_LOG"
    fi
    ...
    if [ -n "$(git -C "$REPO_DIR" status --porcelain)" ]; then
        git -C "$REPO_DIR" commit -m "Add verification log v12"
    fi
    ...
    echo "Scripts v12: $BASE/scripts/stage-opencode-repo-v12.sh"
