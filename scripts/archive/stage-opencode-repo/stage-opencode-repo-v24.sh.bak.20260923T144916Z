...
# v24: streaming telemetry in smoke tests via process substitution + tee;
#      response text assertion preserved; elapsed time reported; no silent
#      buffering. Saves v10-v24 in scripts/.
...
# Streaming docker run: user sees output live as it arrives, and the
# function still captures stdout/stderr for text assertion.
# Process substitution (> >(tee ...)) sends each stream to both the
# terminal and the temp file simultaneously. No silent buffering.
docker_run_assert_text() {
    ...
    echo "  Safety timeout: 120s (fires only if no progress)"
    ...
    if echo "$stdout" | grep -q "$expected"; then
        echo "  PASS: stdout contains '$expected' (exit code $exit_code informational)"
        return 0
    fi
    if echo "$stderr" | grep -q "$expected"; then
        echo "  PASS: stderr contains '$expected' (exit code $exit_code informational)"
        return 0
    fi
...
# Streaming docker run for commands that are not asserted on text.
docker_run_streaming() {
    echo "  Running: docker run --rm -i --user $(id -u):$(id -g) ... $*"
    echo "  Safety timeout: 60s"
    echo "  --- begin live telemetry ---"
...
GITIGNORE_EOF
    ...
    # 13. BEHAVIORAL SMOKE TEST: DeepSeek (streaming + text assertion)
    echo ""
    ...
    # 14. BEHAVIORAL SMOKE TEST: Plugins (streaming + text assertion)
    echo ""
...
PY_EOF
    ...
    if [ -n "$(git -C "$REPO_DIR" status --porcelain)" ]; then
        if git -C "$REPO_DIR" commit -m "OpenCode + DeepSeek V4.1 Flash + Jev: v24"; then
    ...
    echo "REPORT 7: streaming via process substitution" | tee -a "$VERIFY_LOG"
    if grep -q 'tee "\$tmp_out"' "$REPO_DIR/stage-opencode-repo-v24.sh"; then
        echo "  PASS" | tee -a "$VERIFY_LOG"
    else
    ...
    echo "REPORT 13: scripts/ contains v24" | tee -a "$VERIFY_LOG"
    if [ -f "$SCRIPTS_DIR/stage-opencode-repo-v24.sh" ]; then echo "  PASS" | tee -a "$VERIFY_LOG"; else echo "  FAIL" | tee -a "$VERIFY_LOG"; fi
    ...
    echo "REPORT 15: elapsed time reported in smoke test" | tee -a "$VERIFY_LOG"
    if grep -q "Elapsed:" "$REPO_DIR/stage-opencode-repo-v24.sh"; then echo "  PASS" | tee -a "$VERIFY_LOG"; else echo "  FAIL" | tee -a "$VERIFY_LOG"; fi

    git -C "$REPO_DIR" add verification.log
    if [ -n "$(git -C "$REPO_DIR" status --porcelain)" ]; then
        git -C "$REPO_DIR" commit -m "Add verification log v24"
    fi
    ...
    echo "Scripts v24: $BASE/scripts/stage-opencode-repo-v24.sh"
