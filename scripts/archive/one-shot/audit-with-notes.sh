...
show() { case "$1" in *"message=loading path="*|*"message=tracking hash="*|*"message=\"creating instance\""*|*"message=fromDirectory"*|*"message=bootstrapping"*|*"message=\"project copy refresh"*|*"message=\"watcher backend\""*) return 1 ;; *) return 0 ;; esac; }
cleanup() { [ -n "$CHILD" ] && kill -TERM "$CHILD" >/dev/null 2>&1; [ -n "$STREAMER" ] && kill -TERM "$STREAMER" >/dev/null 2>&1; return 0; }
...
main() {
  ...
  local override="$REPO/docker/docker-compose.override.yml"

  trap cleanup INT TERM

  printf '=== audit-with-notes.sh ===\nrepo: %s\n\n' "$REPO"
  log INFO session START "audit-with-notes" "repo=$REPO" "art=$ART"

  [ -d "$NOTES_SRC" ] || { printf 'GATE FAIL: notes source not at %s\n' "$NOTES_SRC" >&2; return 1; }
  printf '[1] notes source: %s files\n' "$(ls -1 "$NOTES_SRC" | wc -l | tr -d ' ')"

  printf '[2] writing override file\n'
  cat > "$override" <<EOF
services:
  opencode-web:
    volumes:
...
EOF
  ...
  printf '[3] validate merged config\n'
  ( cd "$REPO/docker" && docker compose config >/dev/null 2>&1 )
  if [ $? -ne 0 ]; then
    printf '    FAIL: compose config rejected merged file\n' >&2
    ( cd "$REPO/docker" && docker compose config 2>&1 | head -20 )
    return 1
  fi
  printf '    OK\n'
  ...
  printf '[5] verify mount in inspect\n'
  if docker inspect "$C" --format '{{json .Mounts}}' | grep -qF "\"Destination\":\"$NOTES_CONT\""; then
    printf '    mount present\n'
  else
    printf '    FAIL: inspect has no %s\n' "$NOTES_CONT"
  ...
  printf '    entries: %s\n' "$in_count"
  [ "$in_count" -lt 1 ] && { printf '    FAIL: empty mount\n' >&2; return 1; }

  printf '[6] prompt\n'
  local prompt="$ART/prompt.txt"
  {
    printf 'Audit an OpenCode + DeepSeek + JEV container.\n\n'
    printf 'Locations:\n'
    ...
    printf '1. Project identity from README.txt, QUICKSTART.txt, opencode.json.\n'
    ...
    printf '4. Current state: opencode-web block from docker/docker-compose.yml, docker/web-entrypoint.sh, session count via node --experimental-sqlite.\n'
    printf '5. Sidebar streaming: is a processing session visible in the UI? yes/no with evidence.\n'
  ...
  while kill -0 "$CHILD" >/dev/null 2>&1; do
    ...
    if [ "$el" -ge "$TIMEOUT" ]; then
      kill -TERM "$CHILD" >/dev/null 2>&1; sleep 2
      kill -KILL "$CHILD" >/dev/null 2>&1; break
    fi
  done
